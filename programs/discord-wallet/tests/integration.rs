...
fn discord_transfer_token_to_owner_ata_works() {
    ...
    let recipient_token_account = associated_token_address(&recipient_owner, &mint);
    MintToChecked::new(&mut svm, &relayer, &mint, &vault_token_account, 2_500_000)
        .send()
        .unwrap();
...
fn discord_transfer_token_to_mentioned_user_vault_ata_works() {
    ...
    let sender_token_account = CreateAssociatedTokenAccount::new(&mut svm, &relayer, &mint)
        ...
        .send()
        .unwrap();
    let recipient_token_account = associated_token_address(&recipient_vault, &mint);
...
fn execute_discord_command_with_signature(
    ...
    let mut instructions = token_transfer_setup_instructions(relayer, user_id, name, options);
...
fn token_transfer_setup_instructions(
    relayer: &Keypair,
    _user_id: u64,
    name: &str,
    options: &[(&str, &str)],
    ...
    let mint = solana_address::Address::from_str(token).unwrap();
    let to = option_value(options, "to");
    let owner = if let Some(destination_user_id) = parse_mention(to) {
        let destination_wallet_state = wallet_pda(destination_user_id, &PROGRAM_ID).0;
        vault_pda(&destination_wallet_state, &PROGRAM_ID).0
    } else {
        solana_address::Address::from_str(to).unwrap()
    };
